...
{
    DebugPrintf("Task_NewGameBirchSpeech_WaitToShowBirch");
...
{
    DebugPrintf("Task_NewGameBirchSpeech_WaitForSpriteFadeInWelcome");
...
{
    DebugPrintf("Task_NewGameBirchSpeech_ThisIsAPokemon");
...
{
    DebugPrintf("Task_NewGameBirchSpeech_InitPokeball");
...
{
    DebugPrintf("Task_NewGameBirchSpeech_WaitForLotad");
    DestroyTask(taskId);
}
...
{
    DebugPrintf("Task_NewGameBirchSpeech_AndYouAre");
...
{
    DebugPrintf("Task_NewGameBirchSpeech_StartBirchLotadPlatformFade");
    if (!RunTextPrintersAndIsPrinter0Active())
    {
...
{
    DebugPrintf("Task_NewGameBirchSpeech_SlidePlatformAway");
...
{
    DebugPrintf("Task_NewGameBirchSpeech_StartPlayerFadeIn");
    // if (gTasks[taskId].tIsDoneFadingSprites)
    {
        u8 spriteId;
...
{
    DebugPrintf("Task_NewGameBirchSpeech_WaitForPlayerFadeIn");
    // if (gTasks[taskId].tIsDoneFadingSprites)
    {
...
{
    DebugPrintf("Task_NewGameBirchSpeech_BoyOrGirl");
    NewGameBirchSpeech_ClearWindow(0);
...
{
    DebugPrintf("Task_NewGameBirchSpeech_SoItsPlayerName");
    // NewGameBirchSpeech_ClearWindow(0);
...
{
    DebugPrintf("Task_NewGameBirchSpeech_CreateNameYesNo");
    // if (!RunTextPrintersAndIsPrinter0Active())
    {
...
{
    DebugPrintf("Task_NewGameBirchSpeech_ProcessNameYesNoMenu");
...
{
    // u8 spriteId;
    DebugPrintf("Task_NewGameBirchSpeech_ReshowBirchLotad");
...
{
    // if (gTasks[taskId].tIsDoneFadingSprites)
    DebugPrintf("Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter");
    {
        // if (!RunTextPrintersAndIsPrinter0Active())
        {
            // NewGameBirchSpeech_StartFadeOutTarget1InTarget2(taskId, 2);
...
{
    // u8 spriteId;
    DebugPrintf("Task_NewGameBirchSpeech_AreYouReady");
...
{
    // u8 spriteId;
    DebugPrintf("Task_NewGameBirchSpeech_ShrinkPlayer");
...
static void CB2_NewGameBirchSpeech_ReturnFromNamingScreen(void)
{
    ...
    DebugPrintf("CB2_NewGameBirchSpeech_ReturnFromNamingScreen");
    ResetBgsAndClearDma3BusyFlags(0);
    SetGpuReg(REG_OFFSET_DISPCNT, 0);
    SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_OBJ_ON | DISPCNT_OBJ_1D_MAP);
    // InitBgsFromTemplates(0, sMainMenuBgTemplates, ARRAY_COUNT(sMainMenuBgTemplates));
